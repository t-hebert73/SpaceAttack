...
# - Added addition difficulty level *impossible*
# - Increased Game size to 768x576
# - Updated all difficulty levels



import pygame, gameEngine, random, sys, time
...
class Blackhole(gameEngine.SuperSprite):
    def __init__(self, scene):
        gameEngine.SuperSprite.__init__(self, scene)
        self.setImage("images/blackhole.jpg")
        self.reset()
    ...
    def reset(self):
        ...
        #set random size
        scale = random.randint(30, 40)
        self.setImage("images/blackhole.jpg")
        self.imageMaster = \
            pygame.transform.scale(self.imageMaster, (scale, scale))
        
        self.setSpeed(random.randint(4, 6))
        self.setAngle(random.randint(0, 360))
        self.rotSpeed = random.randint(3, 5)
...
#powerball class - handles powerballs image, position, speed and angle
...
#game class - handles the gameplay when a difficulty is seleced from the gui        
class Game(gameEngine.Scene):
    def __init__(self, difficulty):
        ...
        iconpath = "images/shipthrust.gif"        #
        iconimage = pygame.image.load(iconpath)    # sets the icon image
        ...
        self.difficultylabel = gameEngine.Label()    #create difficulty label
        self.difficultylabel.center = (400, 20)  #place it top middle
        self.difficultylabel.size = (215,30) 
        self.difficultylabel.bgColor = (0x00, 0x00, 0x00)
        self.difficultylabel.text = self.difficulty
        if self.difficulty == "Easy":
            self.difficultylabel.fgColor = (0x00, 0xCC, 0x00)#green
        elif self.difficulty == "Medium":
            self.difficultylabel.fgColor = (0xFF, 0xA5, 0x00)#orange
        elif self.difficulty == "Hard":
            self.difficultylabel.fgColor = (0xCC, 0x00, 0x00)#red
        elif self.difficulty == "Impossible":
            self.difficultylabel.fgColor = (0x8A, 0x2B, 0xE2)#purple
        ...
        self.easynumberblackholes = 2
        self.easynumberrocks = 8
        self.easynumberaliens = 5
        self.easynumberpowerballs = 4
        self.mediumnumberblackholes = 4
        self.mediumnumberrocks = 10
        self.mediumnumberaliens = 8
        self.mediumnumberpowerballs = 4
        self.hardnumberblackholes = 6
        self.hardnumberrocks = 14
        self.hardnumberaliens = 11
        self.hardnumberpowerballs = 3
        self.impossiblenumberblackholes = 9
        self.impossiblenumberrocks = 16
        self.impossiblenumberaliens = 14
        self.impossiblenumberpowerballs = 2
        ...
        if self.difficulty == "Easy":
            ...
            for i in range(self.impossiblenumberrocks): #make medium number rocks
                self.rocks.append(Rock(self))
            for i in range(self.impossiblenumberaliens): #make medium number rocks
                self.aliens.append(Alien(self))
            for i in range(self.impossiblenumberblackholes): #make medium number rocks
                self.blackholes.append(Blackhole(self))
            for i in range(self.impossiblenumberpowerballs): #make hard number of powerballs
                self.powerballs.append(Powerball(self))
        ...
        self.sprites = [self.bullet, self.bullet2, self.bullet3, self.bullet4,
                        self.bullet5, self.ship, self.scoreboard, self.difficultylabel, self.healthBar]
...
class Gui(gameEngine.Scene):
    def __init__(self):
        ...
        self.addButtons()
        self.highScore = 100
        self.sprites = [self.title, self.highscorelabel, self.scorelabel, self.scoredifficultylabel, self.objectivelabel, self.instructions, self.playagain, self.easyButton, self.mediumButton, self.hardButton, self.impossibleButton, self.quitButton]
        
        
    def addLabels(self):    #create title label and set attributes
        self.title = gameEngine.Label()
        self.title.font = pygame.font.Font("goodfoot.ttf", 55)
        self.title.text = "Space Attack!"
        self.title.fgColor = (0xCC, 0xCC, 0xCC) #white
        self.title.bgColor = (0x00, 0x00, 0x00) #black
        self.title.center = (380, 40)   # top middle
        self.title.size = (270, 70)
        ...
        self.highscorelabel.center = (456, 110) # topleft
        ...
        self.scorelabel.center = (615, 110) # topright
        ...
        self.scoredifficultylabel.center = (625, 160) # topright
        self.scoredifficultylabel.size = (190, 50)
        
        self.objectivelabel = gameEngine.MultiLabel()
        self.objectivelabel.font = pygame.font.Font("goodfoot.ttf", 32)
        ...
        self.objectivelabel.center = (160, 235) # topright
        self.objectivelabel.size = (270, 280)

        self.playagain = gameEngine.MultiLabel()
        self.playagain.font = pygame.font.Font("goodfoot.ttf", 50)
        ...
        self.playagain.center = (630, 320)   # top middle
        self.playagain.size = (250, 130)
        
        self.instructions = gameEngine.MultiLabel() #set up instructions label
        self.instructions.font = pygame.font.Font("goodfoot.ttf", 35)
    ...
    def addButtons(self):   #add the GUI buttons
        ...
        self.hardButton.center = (400, 320)
        self.hardButton.text = "Hard"   #hard button
        self.hardButton.bgColor = (0xCC, 0x00, 0x00) #red
        
        self.impossibleButton = gameEngine.Button()
        self.impossibleButton.center = (400, 360)
        self.impossibleButton.text = "Impossible"   #impossible button
        self.impossibleButton.bgColor = (0x8A, 0x2B, 0xE2) #purple
        
        
        self.quitButton = gameEngine.Button()
        self.quitButton.center = (400, 500)
    ...
    def update(self):
        ...
        if self.impossibleButton.clicked: #if hard button was clicked
            game = Game("Impossible")
            self.guisound.stop() #stop guisound
            self.gamesound.play(-1) # play gamesound
            game.start()
            self.gamesound.stop() #stop gamesound
            ...
            if gameScore > self.highScore:
                self.highScore = gameScore
                self.scoredifficultylabel.fgColor = (0x8A, 0x2B, 0xE2) #possible
                self.scoredifficultylabel.text = "On Impossible"
            scoreString = self.highScore.__str__()
            self.scorelabel.text = scoreString
...
#splash screen class - imitates a loading screen  
class SplashScreen(gameEngine.Scene):
    def __init__(self):
        ...
        iconimage = pygame.image.load(iconpath)    # sets the icon image
        self.setIcon(iconimage)                  #
        self.setCaption("Game Loading")
        self.timer = 0  #set the timer 
        self.clock = pygame.time.Clock()    #create a clock for the timer
        
        self.backgroundimage = gameEngine.SuperSprite(self) #create the background image
        self.backgroundimage.setImage("images/gameloading.gif") #grab the image
        self.backgroundimage.x = 380    #set image x
        self.backgroundimage.y = 290    #set image y
        self.timeToWait = 1 #set amount of time splashscreen is shown
        self.sprites = [self.backgroundimage]
        
        
    def startGame(self):    #starts the game gui
        self.stop()
        gui = Gui()
        gui.start()
    ...
    def update(self):
        seconds = self.clock.tick(10)/1000.0    #grab the seconds
        self.timer += seconds   #add the seconds to the timer
        self.truncatetime = self.timer.__trunc__()  #get rid of decimal places
        if(self.truncatetime > self.timeToWait):  #when the timer reaches x seconds game is 'loaded'
            self.startGame()    #start the game gui

#main 
def main():
    splashScreen = SplashScreen()   #create a splashscreen
    splashScreen.start()    #start the splashscreen
